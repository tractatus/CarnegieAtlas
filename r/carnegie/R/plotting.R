data(C18heart, envir=environment())

glassheart<-function(dataset, high.res=FALSE, dim=c(720,1080), device=TRUE, col='region', cex=0.5, hemisphere='right', spheres=FALSE, alpha=1, laterality=TRUE, plane='coronal'){
  ...
  #par3d(persp)
  if(high.res){
    drawScene.rgl(list(heart18smp3))
  }else{
    drawScene.rgl(list(heart18smp8))
  }
